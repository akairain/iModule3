var Attachment = {
	...
	insertWysiwyg:{
		image:function(id,file,editor) {
			...
			var sHTML = '<img src="'+file.path+'" data-idx="'+file.idx+'" alt="'+file.name+'">';
			...
			editor.redactor("observe.images");
		...
		download:function(id,file,editor) {
			...
			var sHTML = '<a data-idx="'+file.idx+'" class="btn btnWhite btnDownload"><i class="fa fa-download"></i> '+file.name+' <span class="filesize">('+Attachment.getFileSize(file.size)+')</span></a>&nbsp;';
			if ($(editor.redactor("selection.getCurrent")).parents("p").length > 0) {
				editor.redactor("insert.htmlWithoutClean",sHTML);
			} else {
				editor.redactor("insert.htmlWithoutClean",'<p>'+sHTML+'</p>');
